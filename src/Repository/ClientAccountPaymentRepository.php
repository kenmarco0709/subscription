...
/**
 * ClientAccountPaymentRepository
 *
 * This class was generated by the Doctrine ORM. Add your own custom
 * repository methods below.
 */
class ClientAccountPaymentRepository extends \Doctrine\ORM\EntityRepository
{

    public function validate($clientAccountPayment_form) {

        $errors = array();

        $action = $clientAccountPayment_form['action'];

        $paymentDate = $clientAccountPayment_form['paymentDate'];

        // d = delete
        if($action !== 'd') {
            $transactionNoExist = $this->getEntityManager()->getRepository(ClientAccountPaymentEntity::class)
            ->createQueryBuilder('cmp')
            ->where('cmp.id !=  :id')
            ->andWhere('cmp.transactionNo =:transactionNo')
            ->setParameters(array(
                'transactionNo' => $clientAccountPayment_form['transactionNo'],
                'id' => $clientAccountPayment_form['id']
            ))
            ->getQuery()->getResult();
            ...
            if(empty($clientAccountPayment_form['payment_type_desc'])) {
                $errors[] = 'Payment type is required';
            }

            if(empty($clientAccountPayment_form['paymentDate'])) {
                $errors[] = 'Payment date is required';
            }

            if(empty($clientAccountPayment_form['amount'])) {
                $errors[] = 'Amount is required';
            }

            if($clientAccountPayment_form['amount'] > $clientAccountPayment_form['amount_tendered']) {
    ...
    public function ajax_list(array $get, $userData){
        ...
        $columns = array(
            ...
            array('cmp.`parsed_file_description`', 'cmp.`parsed_file_description`', 'parsedFileDescription'),
        ...
        $select = "SELECT";
        $from = "FROM `client_account_payment` cmp";
        ...
        if($userData['type'] != 'Super Admin' || $get['clientAccountId']){
            ...
            $sqlWhere .= " AND cmp.`client_account_id` = :clientAccountId";
        ...
        $url = $get['url'];
        $formUrl = '';
        $printReceiptUrl = 'client_account_payment/print/receipt/';
        $hasUpdate = false;
        $hasDetails = false;
        $hasPrintReceipt = false;

        
        if($userData['type'] == 'Super Admin'  || in_array('Client Details Account Details Payment Update', $userData['accesses'])){
            $hasUpdate = true;
        }

        if($userData['type'] == 'Super Admin'  || in_array('Client Details Account Details Payment Print Receipt', $userData['accesses'])){
        ...
        foreach($result as $k => $row) {
            ...
            $values = array(
                ...
                $row['amount'],
                $row['paymentType'],
                '<a href="'.$url.'uploads/file/' .$row['parsedFileDescription'].'" target="_blank">'.$row['parsedFileDescription'].'</a>',
                $row['refNo'],
                $row['paymentDate'],
                $action 
    ...
    public function getNextTransactionNo(array $userData){
        ...
        $query = $this->getEntityManager()->getConnection()->prepare("
                ...
                FROM `client_account_payment` cmp
                LEFT JOIN `client_account` cm ON cm.`id` = cmp.`client_account_id`
       ...
       if($dateFrom != 'NULL' && $dateTo != 'NULL'){
        ...
        $query = $this->getEntityManager()->getConnection()->prepare("
                SELECT 
                    ...
                    cm.`description` AS clientAccount,
